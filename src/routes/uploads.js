...
// GET /uploads — lists every uploaded image (admin-only: this is a
// management view over the admin's own uploads, not public post content).
router.get('/', requireAuth, async (req, res) => {
  try {
    const images = await listImages();
    res.json(images);
  } catch (err) {
    console.error('GET /uploads failed:', err);
...
// DELETE /uploads/:name — name is just the filename generated by
// uploadImage() (no slashes), not a full path — deleteImage() rejects
// anything else.
router.delete('/:name', requireAuth, async (req, res) => {
  try {
    const deleted = await deleteImage(req.params.name);
    if (!deleted) return res.status(404).json({ error: 'Imagem não encontrada.' });
    res.status(204).send();
  } catch (err) {
    console.error('DELETE /uploads/:name failed:', err);
